...
func NewDatabaseInterface(dsn string) DatabaseInterface {
	pCl, err := NewPostgresConn(dsn)
	if err != nil {
...
func (db *databaseInterface) CreateItem(ctx context.Context, item *pb.TestItem) (*pb.TestItem, error) {
	...
	_, err = tx.Exec(
		ctx,
		"INSERT INTO event_outbox (aggregatetype, aggregateid, payload) VALUES ($1, $2, $3)",
		"items.created",
		item.Id,
		wireEvent,
	)
