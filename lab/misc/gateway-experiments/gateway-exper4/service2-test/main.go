...
func serveGrpcServer(svr *grpc.Server) {
	...
	if err != nil {
		log.Panic().Err(err).Str("port", "7070").Msg("failed to listen on RPC port")
	}
...
func newHttpGateway() *runtime.ServeMux {
	...
	err := pb.RegisterTestServiceHandlerFromEndpoint(ctx, mux, "localhost:7070", opts)
	if err != nil {
...
func serveHttpGateway(mux *runtime.ServeMux) error {
	return http.ListenAndServe("0.0.0.0:70", mux)
}
